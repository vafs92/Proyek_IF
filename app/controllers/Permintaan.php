...
class Permintaan extends Controller
{
    ...
    public function tambah()
    {
        ...
        if ($data['selectedOption'] == 'barang' && $_POST != null) {
            $kodeP = $_POST['kodeP'];
            var_dump($_POST);
            $this->model('Model_StatusPinjam')->tambahDataPinjamB($_POST) > 0;
            $data['items_barang'] = $this->model('Model_StatusPinjam')->getDataBerhasilB($kodeP);
            $this->view('permintaan/rekam_barang', $data);
    
        } elseif ($data['selectedOption'] == 'ruang' && $_POST != null) {
            $kodeP = $_POST['kodeP'];
            // var_dump($_POST);
            $this->model('Model_StatusPinjam')->tambahDataPinjamR($_POST) > 0;
            $data['items_ruang'] = $this->model('Model_StatusPinjam')->getDataBerhasilR($kodeP);

...
class Model_StatusPinjam
{
    ...
    public function tambahDataPinjamB($data)
    {
        ...
        $this->db->bind(':kodeB', $data['kode']); // Use :kodeB instead of :kode
    ...
    {
        $query = "INSERT INTO table_pinjamruang (kodeP_ruang, namaP, nimP, noWA, kodeR, tanggalP_ruang, jamP_ruang, statusP_ruang) VALUES (:kodeP, :namaP, :nimP, :noWA, :kodeR, :tanggalP, :jamP, 'DIKIRIM')";
        $this->db->query($query);
        $this->db->bind(':kodeP', $data['kodeP']);
        $this->db->bind(':namaP', $data['namaP']);
        $this->db->bind(':nimP', $data['nimP']);
        $this->db->bind(':noWA', $data['noWA']);
        $this->db->bind(':kodeR', $data['kode']);
        $this->db->bind(':tanggalP', $data['tanggalP']);
        $this->db->bind(':jamP', $data['jamP']);
        $this->db->execute();
        return $this->db->rowCount();
    ...
    {
        $query = "SELECT pb.kodeP_barang, pb.tanggalP_barang, pb.kodeB, b.namaB, pb.jamP_barang, pb.statusP_barang  FROM table_pinjambarang AS pb NATURAL JOIN table_barang as b WHERE kodeP_barang = :kodeP";
        $this->db->query($query);
        $this->db->bind(':kodeP', $kodeP);
        return $this->db->resultSet();
    }
    ...
    {
        $query = "SELECT pb.kodeP_ruang, pb.tanggalP_ruang, pb.kodeR, b.namaR, pb.jamP_ruang, pb.statusP_ruang  FROM table_pinjamruang AS pb NATURAL JOIN table_ruang as b WHERE kodeP_ruang = :kodeP";
        $this->db->query($query);
        $this->db->bind(':kodeP', $kodeP);
        return $this->db->resultSet();
    }
